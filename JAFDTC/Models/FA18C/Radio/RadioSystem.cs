// ********************************************************************************************************************
//
// RadioSystem.cs -- fa-18c radio system
//
...
namespace JAFDTC.Models.FA18C.Radio
{
    public enum Radios
    {
        COMM1 = 0,
    ...
    public class RadioSystem : RadioSystemBase<RadioPreset>, ISystem
    {
        ...
        public override bool IsDefault
        {
            get
            {
                foreach (ObservableCollection<RadioPreset> presets in Presets)
                    if (presets.Count > 0)
                        return false;
                return string.IsNullOrEmpty(COMM1DefaultTuning) && string.IsNullOrEmpty(COMM2DefaultTuning);
            }
        }
        ...
        /// as the modulation for the hornet is fixed and can't change from what is already in the dtc.
        /// </summary>
        private static void MergeRadioIntoSimDTC(JsonNode radioRoot, ObservableCollection<RadioPreset> presets)
        {
            foreach (RadioPreset preset in presets)
            {
                ...
                if (double.TryParse(preset.Frequency, out double freq))
                    dtcPreset["frequency"] = Math.Truncate(freq * 1000.0) / 1000.0;
            }
        }
